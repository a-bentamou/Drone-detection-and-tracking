...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, opt.mode, imgsz, batch_size,
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        Mode = cv2.IMREAD_COLOR if self.mode=='RGB' else cv2.IMREAD_GRAYSCALE
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, mode='RGB', img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = cv2.imread(path, Mode)  # 0 Gray # BGR
