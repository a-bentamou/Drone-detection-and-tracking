...
def detect(save_img=False):
    source, weights, view_img, save_txt, imgsz, trace,blur, classify = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace,opt.blur, opt.classify
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    if classify:
                        cv2.putText(im0, labels[pred_distorsion], (20,30), 0, 1, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
                    if int(cls) == 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/cluster/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--download', action='store_true', help='download model weights automatically')
    parser.add_argument('--no-download', dest='download', action='store_false')
    parser.add_argument('--source', type=str, default='inference/videos_birds', help='source')  # file/folder, 0 for webcam  inference/images
    ...
    parser.add_argument('--classes', nargs='+', type=int, help='filter by class: --class 0, or --class 1')
    parser.add_argument('--agnostic-nms', action='store_true', help='class-agnostic NMS')
    parser.add_argument('--augment', action='store_true', help='augmented inference')
    parser.add_argument('--update', action='store_true', help='update all models')
    parser.add_argument('--project', default='runs/detect', help='save results to project/name')
    parser.add_argument('--name', default='Drone_detection', help='save results to project/name')
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment')
    parser.add_argument('--no-trace', action='store_true', help='don`t trace model')
    parser.add_argument('--blur', action='store_true', help='blur detections')
    parser.add_argument('--classify', action='store_true', default=False, help='distrotion detection')
